...
We propose \textbf{MemAD}, which augments MomAD's Momentum Planning Interactor (MPI) with an \emph{Adaptive History Selector} (AHS), a lightweight MLP-based gating network that dynamically selects among three history depths ($d \in \{0, 1, 2\}$) conditioned on ego-vehicle kinematics, the current planning query, and the driving command.
...
MomAD~\cite{li2025momad} tackles this problem with \emph{momentum planning}, introducing Topological Trajectory Matching (TTM) and a Momentum Planning Interactor (MPI) that fuses the current planning query with one previous frame's query and score via a Long-Horizon Query Mixer.
While effective, the MPI uses a \emph{fixed} history depth.
The MomAD ablation study on Turning-nuScenes reveals a \emph{history paradox}: using $t{=}2$ frames of history yields the best results, while $t{=}3$ degrades performance below $t{=}2$, indicating that the optimal history depth is not constant across scenarios.
...
To test this, we propose \textbf{MemAD}, which inserts an \emph{Adaptive History Selector} (AHS) upstream of the Long-Horizon Query Mixer.
...
Mixture-of-experts architectures~\cite{shazeer2017outrageously} use learned gating networks to route inputs to specialized sub-networks.
Our AHS applies a similar principle: rather than routing to different expert networks, it routes to different \emph{temporal depths}, selecting how much historical context to use for each planning step.
...
MomAD builds on SparseDrive's multi-task architecture comprising a ResNet-50 backbone with FPN~\cite{lin2017fpn}, a sparse 3D detection head, a vectorized map head, and a joint motion-planning head.
...
\begin{equation}
    ...
    \label{eq:mixer}
\end{equation}
where $\sigma$ denotes the sigmoid function and the LSTM~\cite{hochreiter1997lstm} (hidden dimension 128) maintains temporal coherence across modes.
...
We use AdamW~\cite{loshchilov2019adamw} with a base learning rate of $2 \times 10^{-5}$ and weight decay $10^{-3}$.
The AHS and Long-Horizon Query Mixer receive a $20\times$ learning rate multiplier ($4 \times 10^{-4}$) to allow rapid adaptation while the pretrained backbone (ResNet-50~\cite{he2016resnet}) uses a $0.1\times$ multiplier.
...
Stratifying by navigation command reveals a \emph{bimodal turn policy}: during right turns, the gate selects no-history (branch~0) 9.8\% of the time, nearly 5$\times$ the rate during straight driving (2.2\%).
...
Closed-loop benchmarks such as Bench2Drive~\cite{jia2024bench2drive} would test whether adaptive history selection translates to improved reactive driving.
