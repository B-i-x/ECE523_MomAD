...
class NextTokenPredictor(nn.Module):
    ...
    def forward(self, last_feat, last_score, cur_feat):
        weighted_last_feat = last_feat * last_score.sigmoid().unsqueeze(-1)
        combined_feat = cur_feat + weighted_last_feat
        combined_feat = combined_feat.squeeze(1)
        lstm_out = self.lstm.to(last_feat.device)(combined_feat)
        return lstm_out
